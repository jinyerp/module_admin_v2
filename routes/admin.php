...
->group(function () {
    ...
    // 관리자 등급 관리
    Route::prefix('levels')->name('levels.')->middleware(['web', 'admin:auth'])
    ->group(function () {
        Route::get('/', [
        ...
        Route::get('/{id}', [
            AdminLevelController::class, 
            'show'])->name('show')->where('id', '[0-9]+');
        Route::get('/{id}/edit', [
            AdminLevelController::class, 
            'edit'])->name('edit')->where('id', '[0-9]+');
        Route::put('/{id}', [
            AdminLevelController::class, 
            'update'])->name('update')->where('id', '[0-9]+');
        Route::delete('/{id}', [
            AdminLevelController::class, 
            'destroy'])->name('destroy')->where('id', '[0-9]+');
        Route::get('/{id}/confirm', [
            AdminLevelController::class, 
            'deleteConfirm'])->name('confirm')->where('id', '[0-9]+');
        Route::post('/bulk-delete', [
            AdminLevelController::class, 
            'bulkDelete'])->name('bulk-delete');
    });
...
->group(function () {
    ...
    Route::post('/bulk-delete', [AdminSessionController::class, 'bulkDelete'])->name('bulk-delete'); // 일괄 삭제
});
...
Route::prefix('admin/admin')->name('admin.admin.')->middleware(['web', 'admin:auth'])
->group(function () {
...
Route::prefix('admin/admin')->middleware(['web', 'admin:auth'])
    ...
    // 2FA 로그 관리 - 2fa-logs
    Route::prefix('user-2fa-logs')->name('user-2fa-logs.')->group(function () {
...
Route::prefix("$adminPrefix/admin")->middleware(['web', 'admin:auth'])->name('admin.admin.')->group(function () {
    ...
    // 권한 로그 관리 - permission-logs (단순화된 버전)
...
Route::prefix($adminPrefix)->middleware(['web', 'admin:auth'])->name('admin.')
    ...
    Route::get('/systems', [
        ...
        'index'])->name('systems.index');
    
    // 시스템 정보 API
    Route::get('/systems/status', [
        AdminSystemController::class,
        'status'])->name('systems.status');
    
    // PHP 상세 정보
    Route::get('/systems/php', [
        AdminSystemController::class,
        'phpDetail'])->name('systems.php');
    
    // Laravel 상세 정보
    Route::get('/systems/laravel', [
        AdminSystemController::class,
        'laravelDetail'])->name('systems.laravel');
    
    // 데이터베이스 상세 정보
    Route::get('/systems/database', [
        AdminSystemController::class,
        'databaseDetail'])->name('systems.database');
    
    // 세션 상세 정보
    Route::get('/systems/session', [
        AdminSystemController::class,
        'sessionDetail'])->name('systems.session');
});


// 시스템 메뉴 관리
use Jiny\Admin\App\Http\Controllers\Admin\AdminSideMenuController;
Route::prefix('/admin/system/menu')->name('admin.system.menu.')->group(function () {
