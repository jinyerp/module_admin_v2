...
@section('title', '등급 관리')
@section('description', '시스템에서 사용하는 등급 목록을 관리합니다. 등급명, 등급코드, 배지 색상, 권한 설정 등을 관리할 수 있습니다.')
    ...
    <div class="w-full">
        <div class="sm:flex sm:items-end justify-between">
            <div class="sm:flex-auto">
                ...
                <p class="mt-2 text-base text-gray-700">시스템에서 사용하는 등급 목록을 관리합니다. 등급명, 등급코드, 배지 색상, 권한 설정 등을 관리할 수 있습니다.</p>
            </div>
...
@section('content')
    ...
    <x-ui::table-stripe>
        <x-ui::table-thead>
            <x-ui::table-th sort="name">등급명</x-ui::table-th>
            <x-ui::table-th sort="code">등급코드</x-ui::table-th>
            <x-ui::table-th sort="badge_color">배지 색상</x-ui::table-th>
            <x-ui::table-th sort="can_create">생성 권한</x-ui::table-th>
            <x-ui::table-th sort="can_read">조회 권한</x-ui::table-th>
            <x-ui::table-th sort="can_update">수정 권한</x-ui::table-th>
            <x-ui::table-th sort="can_delete">삭제 권한</x-ui::table-th>
        ...
        <tbody class="bg-white">
            @foreach ($rows as $item)
                <x-ui::table-row :item="$item" data-row-id="{{ $item->id }}"
                    ...
                    <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">
                        @if($item->badge_color)
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $item->badge_color }} text-white">
                                {{ $item->badge_color }}
                            </span>
                        @else
                            -
                    ...
                    <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">
                        @if ($item->can_create)
                            <x-ui::badge-success text="권한 있음" />
                        @else
                            <x-ui::badge-warning text="권한 없음" />
                        @endif
                    </td>
                    ...
                    <td class="relative py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-3">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <span
                                ...
                                data-delete-route="{{ route($route.'destroy', $item->id) }}" title="삭제">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
