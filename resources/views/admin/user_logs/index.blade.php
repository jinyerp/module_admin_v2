...
@section('title', '관리자 로그인 로그 목록')
@section('description', '관리자 로그인 로그를 확인하고 관리할 수 있습니다.')

@section('heading')
    <div class="w-full">
        <div class="sm:flex sm:items-end justify-between">
            <div class="sm:flex-auto">
                <h1 class="text-2xl font-semibold text-gray-900">로그인 로그 목록</h1>
                <p class="mt-2 text-base text-gray-700">관리자 로그인 로그를 확인하고 관리할 수 있습니다. 로그인 시도, 성공/실패 상태, IP 주소 등을 추적할 수 있습니다.</p>
            </div>
...
@section('content')
    ...
    <div class="mt-6 bg-white rounded-lg border border-gray-200 p-4">
        <div id="filter-container" class="space-y-4">
            ...
            @includeIf('jiny-admin::admin.user_logs.filters')
    ...
    <x-ui::table-stripe>
        <x-ui::table-thead>
            <th class="relative py-3.5 pl-4 pr-3 sm:pl-3">
                <input type="checkbox" id="bulk-checked-all" data-select-all class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600">
            </th>
            ...
            <x-ui::table-th sort="message">메시지</x-ui::table-th>
        ...
        <tbody class="bg-white">
            @foreach ($rows as $log)
                <x-ui::table-row :item="$log" data-row-id="{{ $log->id }}"
                    ...
                    <td class="relative py-4 pl-4 pr-3 sm:pl-3">
                        <input type="checkbox" value="{{ $log->id }}" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600">
                    </td>
                    ...
                    <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">
                        ...
                        @elseif($log->status === 'blocked')
                            <x-ui::badge-warning text="차단" />
                        @else
                            <x-ui::badge-primary text="{{ $log->status_label }}" />
                        @endif
                    </td>
                    ...
                    <td class="relative py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-3">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <a href="{{ route($route.'edit', $log->id) }}"
                                class="text-indigo-600 hover:text-indigo-900 p-1 rounded-md hover:bg-indigo-50 transition-colors"
                                title="수정">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2"
                                    viewBox="0 0 24 24">
                                ...
                                <span class="sr-only">Edit log {{ $log->id }}</span>
                            </a>
                            <span
                                class="text-red-600 hover:text-red-900 p-1 rounded-md hover:bg-red-50 transition-colors cursor-pointer"
                                data-delete-route="{{ route($route.'destroy', $log->id) }}" title="삭제">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                </svg>
                                <span class="sr-only">Delete log {{ $log->id }}</span>
                            </span>
                        </div>
                    </td>
                </x-ui::table-row>
