...
@section('title', '새 백업 로그 등록')
@section('description', '새로운 백업 로그 정보를 입력하고 등록하세요.')
    ...
    <div class="w-full">
        <div class="sm:flex sm:items-end justify-between">
            <div class="sm:flex-auto">
                ...
                <p class="mt-2 text-base text-gray-700">시스템에서 지원하는 백업 로그 목록을 관리합니다. 백업 타입, 상태, 파일 정보 등을 관리할 수 있습니다.</p>
            </div>
...
@section('content')
    ...
    <form action="{{ route('admin.systems.backup-logs.store') }}" method="POST" class="mt-6" id="create-form" data-list-url="{{ route('admin.systems.backup-logs.index') }}">
        @csrf
        <div class="space-y-12">
            <x-ui::form-section
                title="기본 정보"
                description="백업 로그의 기본 정보를 입력하세요.">
                        ...
                        <div class="mt-2 relative">
                            <select name="backup_type" id="backup_type" 
                                ...
                                @foreach($backupTypes ?? [] as $key => $value)
                                    <option value="{{ $key }}" {{ old('backup_type') == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <x-ui::form-section
                title="파일 정보"
                description="백업 파일의 정보를 입력하세요.">
                <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6 md:col-span-2">
                    <div class="sm:col-span-6">
                        <label for="file_path" class="block text-sm font-medium text-gray-700 mb-1">파일 경로</label>
                        <div class="mt-2 relative">
                            <input type="text" name="file_path" id="file_path" value="{{ old('file_path') }}"
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <div class="mt-2 relative">
                            <input type="text" name="checksum" id="checksum" value="{{ old('checksum') }}"
            ...
            <x-ui::form-section
                ...
                <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6 md:col-span-2">
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <div class="mt-2 relative">
                            <input type="number" name="duration_seconds" id="duration_seconds" value="{{ old('duration_seconds') }}"
            ...
            <x-ui::form-section
                title="추가 설정"
                description="백업의 추가 설정을 입력하세요.">
                <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6 md:col-span-2">
                    <div class="sm:col-span-3">
                        <div class="flex items-center">
                            <input type="checkbox" name="is_compressed" id="is_compressed" value="1" {{ old('is_compressed') ? 'checked' : '' }}
                    ...
                    <div class="sm:col-span-3">
                        <div class="flex items-center">
                            <input type="checkbox" name="is_encrypted" id="is_encrypted" value="1" {{ old('is_encrypted') ? 'checked' : '' }}
                                class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" />
                    ...
                    <div class="sm:col-span-6">
                        ...
                        <div class="mt-2 relative">
                            <textarea name="error_message" id="error_message" rows="3"
                                ...
                                aria-describedby="error_message-error" placeholder="에러 메시지">{{ old('error_message') }}</textarea>
                    ...
                    <div class="sm:col-span-6">
                        ...
                        <div class="mt-2 relative">
                            <textarea name="metadata" id="metadata" rows="4"
                                ...
                                aria-describedby="metadata-error" placeholder='{"key": "value"}'>{{ old('metadata') }}</textarea>
