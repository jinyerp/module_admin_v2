...
@section('description', '관리자 활동 로그 정보를 수정합니다.')
...
@section('content')
    <div class="pt-2 pb-4">
        <div class="w-full">
            <div class="sm:flex sm:items-end justify-between">
                <div class="sm:flex-auto">
                    <h1 class="text-2xl font-semibold text-gray-900">활동 로그 수정</h1>
                    <p class="mt-2 text-base text-gray-700">관리자 활동 로그 정보를 수정합니다.</p>
                </div>
        ...
        <form action="{{ route($route.'update', $activityLog->id) }}" method="POST" class="mt-6" id="edit-form">
            @csrf
            @method('PUT')
            <div class="space-y-12">
                <x-ui::form-section title="기본 정보" description="로그의 주요 정보를 수정하세요.">
                    <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6 md:col-span-2">
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <input type="text" name="action" id="action" value="{{ old('action', $activityLog->action) }}" class="block w-full rounded-md bg-white px-3 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm @error('action') outline-red-300 focus:outline-red-500 @enderror" required placeholder="액션" />
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <input type="text" name="module" id="module" value="{{ old('module', $activityLog->module) }}" class="block w-full rounded-md bg-white px-3 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm @error('module') outline-red-300 focus:outline-red-500 @enderror" required placeholder="모듈명" />
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <input type="text" name="description" id="description" value="{{ old('description', $activityLog->description) }}" class="block w-full rounded-md bg-white px-3 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm @error('description') outline-red-300 focus:outline-red-500 @enderror" required placeholder="설명" />
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <input type="text" name="ip_address" id="ip_address" value="{{ old('ip_address', $activityLog->ip_address) }}" class="block w-full rounded-md bg-white px-3 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm @error('ip_address') outline-red-300 focus:outline-red-500 @enderror" placeholder="IP 주소" />
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <select name="severity" id="severity" class="block w-full rounded-md bg-white px-3 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm @error('severity') outline-red-300 focus:outline-red-500 @enderror" required>
                                <option value="low" @selected(old('severity', $activityLog->severity)=='low')>낮음</option>
                                <option value="medium" @selected(old('severity', $activityLog->severity)=='medium')>보통</option>
                                <option value="high" @selected(old('severity', $activityLog->severity)=='high')>높음</option>
                                <option value="critical" @selected(old('severity', $activityLog->severity)=='critical')>심각</option>
                            </select>
            ...
            <div class="mt-6 flex items-center justify-end gap-x-6">
                ...
                <x-ui::button-primary type="submit">수정</x-ui::button-primary>
            </div>
        </form>
    </div>
@endsection 