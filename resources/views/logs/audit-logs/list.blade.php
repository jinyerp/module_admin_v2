<x-table-stripe>
    <thead>
        <tr>
            ...
            <x-table-th key="table_name" :sort="$sort" :dir="$dir">테이블</x-table-th>
        ...
        @forelse($logs as $item)
            <tr class="even:bg-gray-50">
                ...
                <td class="px-3 py-4 text-sm whitespace-nowrap">
                    <div class="flex items-center">
                        <a href="{{ route('admin.admin.logs.audit.show', $item->id) }}" class="text-indigo-600 hover:text-indigo-900 font-medium">
                ...
                <td class="relative py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-3">
                    <div class="flex justify-end gap-2">
                        <a href="{{ route('admin.admin.logs.audit.show', $item->id) }}"
                            class="text-indigo-600 hover:text-indigo-900">
                            보기<span class="sr-only">, {{ $item->action }}</span>
                        </a>
                        <a href="{{ route('admin.admin.logs.audit.edit', $item->id) }}"
                ...
                <td colspan="7" class="px-3 py-12 text-center">
                    <div class="flex flex-col items-center">
                        ...
                        <h3 class="text-lg font-medium text-gray-900 mb-2">
                            @if (request('filter_search') || request('filter_admin_id') || request('filter_action') || request('filter_table_name') || request('filter_severity'))
                                검색 결과가 없습니다
                            @else
                                등록된 감사 로그가 없습니다
                            @endif
                        ...
                        @if (request('filter_search') || request('filter_admin_id') || request('filter_action') || request('filter_table_name') || request('filter_severity'))
                            <a href="{{ route('admin.admin.logs.audit.index') }}">
                                <x-link-secondary href="{{ route('admin.admin.logs.audit.index') }}">검색 조건 초기화</x-link-secondary>
                            </a>
                        @else
                            <a href="{{ route('admin.admin.logs.audit.create') }}">
