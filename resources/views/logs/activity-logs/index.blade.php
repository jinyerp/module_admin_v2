...
@section('title', '관리자 활동 로그')
@section('description', '관리자의 모든 활동을 기록하고 조회할 수 있습니다. 로그인, 로그아웃, CRUD 작업 등의 활동 이력을 확인할 수 있습니다.')
...
@section('content')
    ...
    <div class="px-4 sm:px-6 lg:px-8">
        ...
        <x-resource-heading
            title="관리자 활동 로그"
            subtitle="관리자의 모든 활동을 기록하고 조회할 수 있습니다. 로그인, 로그아웃, CRUD 작업 등의 활동 이력을 확인할 수 있습니다.">

            <x-link-primary href="{{ route('admin.admin.logs.activity.create') }}">
                로그 생성
            </x-link-primary>
        ...
        <!-- 통계 카드 -->
        @includeIf('jiny-admin::logs.activity-logs.status')

        <!-- 검색 폼 -->
        <x-resource-search>
            @includeIf('jiny-admin::logs.activity-logs.filter')
        </x-resource-search>

        {{-- 동작/성공 메시지 --}}
        @includeIf('jiny-admin::logs.activity-logs.message')


        @includeIf('jiny-admin::logs.activity-logs.list')
...
@section('modal')
    ...
    <x-modal id="deleteModal">
        <x-resource-row-delete :url="route('admin.admin.logs.activity.destroy', ['activityLog' => ':id'])">
        </x-resource-row-delete>
    </x-modal>

    <!-- 선택삭제 확인 모달 -->
    <x-modal id="bulkDeleteModal">
        <x-resource-bulk-delete :url="route('admin.admin.logs.activity.bulk-delete')">
