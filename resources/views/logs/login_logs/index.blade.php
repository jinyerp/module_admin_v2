...
@section('description', '관리자의 로그인/로그아웃 및 시도 기록을 조회할 수 있습니다. 보안 감사, 이상행위 탐지, 관리자 활동 추적에 활용됩니다.')
...
@section('content')
    ...
    <div class="px-4 sm:px-6 lg:px-8">
        ...
        <x-resource-heading
            title="관리자 사용자 로그"
            subtitle="관리자의 로그인/로그아웃 및 시도 기록을 조회할 수 있습니다. 보안 감사, 이상행위 탐지, 관리자 활동 추적에 활용됩니다.">
        ...
        <!-- 통계 카드 -->
        @includeIf('jiny-admin::logs.login_logs.status')

        <!-- 검색 폼 -->
        <x-resource-search>
            @includeIf('jiny-admin::logs.login_logs.filter')
        </x-resource-search>

        {{-- 동작/성공 메시지 --}}
        @includeIf('jiny-admin::logs.login_logs.message')

        @includeIf('jiny-admin::logs.login_logs.list')
...
@section('modal')
    ...
    <x-modal id="deleteModal">
        <x-resource-row-delete :url="route('admin.admin.logs.user.destroy', ['userLog' => ':id'])">
        </x-resource-row-delete>
    </x-modal>

    <!-- 선택삭제 확인 모달 -->
    <x-modal id="bulkDeleteModal">
        <x-resource-bulk-delete :url="route('admin.admin.logs.user.bulk-delete')">
