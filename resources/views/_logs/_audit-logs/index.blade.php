...
@section('description', '관리자의 모든 활동을 기록하고 조회할 수 있습니다. 생성, 수정, 삭제 등의 작업 이력을 확인할 수 있습니다.')
    ...
    <div class="px-4 sm:px-6 lg:px-8">
        ...
        <div>
            <nav class="sm:hidden" aria-label="Back">
                <a href="{{ route('admin.admin.logs.audit.index') }}" class="flex items-center text-sm font-medium text-gray-500 hover:text-gray-700">
                ...
                <ol role="list" class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('admin.admin.logs.audit.index') }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">감사 로그</a>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <x-resource-heading
            title="관리자 감사 로그"
            subtitle="관리자의 모든 활동을 기록하고 조회할 수 있습니다. 생성, 수정, 삭제 등의 작업 이력을 확인할 수 있습니다.">

            <x-link-primary href="{{ route('admin.admin.logs.audit.create') }}">로그 생성</x-link-primary>

        </x-resource-heading>

        <!-- 통계 카드 -->
        @includeIf('jiny-admin::logs.audit-logs.status')

        <!-- 검색 폼 -->
        <x-resource-search>
            @includeIf('jiny-admin::logs.audit-logs.filter')
        </x-resource-search>

        {{-- 동작/성공 메시지 --}}
        @includeIf('jiny-admin::logs.audit-logs.message')

        @includeIf('jiny-admin::logs.audit-logs.list')

        {{-- 페이지 디버그 --}}
        @includeIf('admin.debug')
...
@section('modal')
    ...
    <x-modal id="deleteModal">
        <x-resource-row-delete :url="route('admin.admin.logs.audit.destroy', ['auditLog' => ':id'])">
        </x-resource-row-delete>
    </x-modal>

    <!-- 선택삭제 확인 모달 -->
    <x-modal id="bulkDeleteModal">
        <x-resource-bulk-delete :url="route('admin.admin.logs.audit.bulk-delete')">
