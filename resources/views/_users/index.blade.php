...
@section('content')
    ...
    <div class="px-4 sm:px-6 lg:px-8">
                ...
                <ol role="list" class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('admin.admin.users.index') }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">관리자 회원 관리</a>
                        </div>
                    </li>
        ...
        <x-resource-heading
            title="관리자 회원"
            subtitle="시스템에서 지원하는 관리자 회원 목록을 관리합니다. 관리자 회원명, 이메일, 타입, 상태 등을 관리할 수 있습니다.">

            <x-link-primary href="{{ route('admin.admin.users.create') }}">관리자 회원 등록</x-link-primary>

        </x-resource-heading>

        <!-- 검색 폼 -->
        <x-resource-search>
            @includeIf('jiny-admin::users.filter')
        </x-resource-search>

        {{-- 동작/성공 메시지 --}}
        @includeIf('jiny-admin::users.message')

        @includeIf('jiny-admin::users.list')
...
@section('modal')
    ...
    {{-- <x-modal id="deleteModal">
        <x-resource-row-delete :url="route('admin.admin.users.destroy', ['id' => ''])" />
    </x-modal> --}}

    <!-- 선택삭제 확인 모달 -->
    {{-- <x-modal id="bulkDeleteModal">
        <x-resource-bulk-delete :url="route('admin.admin.users.bulk-delete')">
