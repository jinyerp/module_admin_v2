...
class SystemBackupLog extends Model
{
     ...
     * 백업 정책 검증 (정적 메서드)
     */
    public static function validateBackupPolicy(): array
    {
        $issues = [];

        // 최근 24시간 내 백업 확인
        $recentBackups = self::where('created_at', '>=', now()->subDay())
            ->where('status', self::STATUS_COMPLETED)
            ->count();

        if ($recentBackups === 0) {
            $issues[] = '최근 24시간 내 성공한 백업이 없습니다.';
        }

        // 암호화되지 않은 백업 확인
        $unencryptedBackups = self::where('is_encrypted', false)
            ->where('status', self::STATUS_COMPLETED)
            ->count();

        if ($unencryptedBackups > 0) {
            $issues[] = '암호화되지 않은 백업이 있습니다.';
        }

        // 실패한 백업 확인
        $failedBackups = self::where('status', self::STATUS_FAILED)
            ->where('created_at', '>=', now()->subDay())
            ->count();

        if ($failedBackups > 0) {
            $issues[] = '최근 24시간 내 실패한 백업이 있습니다.';
        }
